...
const apiFetch = async (path, opts={}) => {
  const r = await fetch(BASE + path, { ...opts, headers: { 'Content-Type':'application/json', Authorization:`Bearer ${tok()}`, ...opts.headers } });
...
const fML = ym => { try { return new Date(ym+'-15').toLocaleString('en-GB',{month:'long',year:'numeric'}); } catch { return ym||'—'; } };
const sum  = (a,k) => a.reduce((s,r)=>s+(parseFloat(r[k])||0),0);
const last = (a,k) => a.length ? (parseFloat(a[a.length-1][k])||0) : 0;
...
export function LPGVoyageList() {
  ...
  useEffect(() => {
    apiFetch('/lpg/vessels')
      .then(v => setVessels(Array.isArray(v) ? v : []))
      .catch(e => console.warn('vessels:', e.message));
  }, []);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      {err && <div className="rounded-lg p-3 bg-red-900/30 border border-red-700/40 text-red-300 text-sm">⚠ {err}</div>}
      ...
      {loading ? <div className="text-slate-500 text-sm py-8 text-center">Loading…</div> : (
        <div className="space-y-6">
          ...
          {!months.length && !err && <div className="text-slate-500 text-sm text-center py-10">No records found. Import an Excel file to get started.</div>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-100">{data.vessel_name} — {fML(data.voyage_number)}</h1>
      ...
      <div className="flex gap-1 rounded-lg p-1 mb-6 overflow-x-auto" style={{background:'rgba(255,255,255,0.04)'}}>
        {[['records','Daily Records'],['fuel','Fuel'],['engine','Engine'],['aux','Aux & Water'],['cii','CII']].map(([t,l]) => (
          ...
          <div className="rounded-xl border border-white/5 overflow-x-auto bg-slate-900/60">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-white/5">
                {['Date','Time','Status','Sea h','Mnv','Anch','NM','RPM','BHP','ULSFO ME','ULSFO AE','ULSFO BLR','ULSFO ROB','Bnkr U','LSMGO','LSMGO ROB','AE1 h','AE2 h','AE3 h','Cargo h','CO₂','FW ROB',''].map(h=>(
              ...
              <tbody>
                {records.map((r,i) => editId===r.id
                  ? <LPGEditRow key={r.id} record={r} onSave={saveRecord} onCancel={()=>setEditId(null)} saving={saving}/>
                  : (
                  <tr key={r.id||i} className={`border-b border-white/[0.03] hover:bg-white/[0.02] ${parseFloat(r.sea_hrs)>0?'':'bg-slate-800/10'}`}>
                    ...
                    <td className="px-2 py-1.5 text-slate-400 max-w-[130px] truncate">{r.status}</td>
                ...
                {newRow && <LPGEditRow key="new" record={newRow} onSave={saveRecord} onCancel={()=>setNewRow(null)} saving={saving} isNew/>}
              </tbody>
              <tfoot><tr className="border-t border-white/10 font-semibold text-xs">
                <td className="px-2 py-3 text-slate-400" colSpan={3}>Totals</td>
...
function FuelTab({records}) {
  ...
  records.forEach(r => {
    ...
    if(!byDate[d]) byDate[d]={label:d.slice(5),ulsfo_me:0,ulsfo_ae:0,ulsfo_blr:0,lsmgo_total:0,ulsfo_rob:0,lsmgo_rob:0};
    ...
    byDate[d].ulsfo_rob    = parseFloat(r.ulsfo_rob)||byDate[d].ulsfo_rob;
    byDate[d].lsmgo_rob    = parseFloat(r.lsmgo_rob)||byDate[d].lsmgo_rob;
  });
      ...
      <div className={CARD}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={cd} margin={{top:0,right:10,left:-10,bottom:0}}>
            ...
            <Bar dataKey="ulsfo_me"  name="ME ULSFO"     stackId="u" fill="#F59E0B"/>
            <Bar dataKey="ulsfo_ae"  name="AE ULSFO"     stackId="u" fill="#3B82F6"/>
            <Bar dataKey="ulsfo_blr" name="Boiler ULSFO" stackId="u" fill="#8B5CF6" radius={[3,3,0,0]}/>
          </BarChart>
        </ResponsiveContainer>
      </div>
      <div className="grid grid-cols-2 gap-6">
        <div className={CARD}>
          <h3 className="text-sm font-semibold text-amber-300 mb-4">Daily LSMGO (MT)</h3>
      ...
      <div className={CARD}>
        ...
        <div className="grid grid-cols-2 gap-6 text-xs">
          <div className="space-y-2">
            {[['ME ULSFO',`${f2(totMe)} MT`],['AE ULSFO',`${f2(totAe)} MT`],['Boiler ULSFO',`${f2(totBlr)} MT`],['Total ULSFO',`${f2(totU)} MT`,'text-amber-300'],['Bunkered ULSFO',`${f2(sum(records,'ulsfo_bunkered'))} MT`,'text-emerald-300'],['ULSFO ROB',`${f2(last(records,'ulsfo_rob'))} MT`,'text-cyan-300']].map(([l,v,c],i)=>(
              <div key={i} className="flex justify-between py-1 border-b border-white/[0.04]"><span className="text-slate-400">{l}</span><span className={`font-mono ${c||'text-slate-200'}`}>{v}</span></div>
            ))}
          </div>
          <div className="space-y-2">
            {[['LSMGO Total',`${f2(totL)} MT`],['Bunkered LSMGO',`${f2(sum(records,'lsmgo_bunkered'))} MT`,'text-emerald-300'],['LSMGO ROB',`${f2(last(records,'lsmgo_rob'))} MT`,'text-cyan-300'],['Sea days',records.filter(r=>parseFloat(r.sea_hrs)>0).length],['Sea hours',`${f1(sum(records,'sea_hrs'))} hrs`],['Total distance',`${f0(sum(records,'obs_dist'))} NM`]].map(([l,v,c],i)=>(
              <div key={i} className="flex justify-between py-1 border-b border-white/[0.04]"><span className="text-slate-400">{l}</span><span className={`font-mono ${c||'text-slate-200'}`}>{v}</span></div>
            ))}
          </div>
        </div>
...
function AuxTab({records}) {
  const wi = records.map((r,i) => {
    ...
    return {
      ...
      ae1d: Math.max(0,(parseFloat(r.ae1_rhr)||0)-(parseFloat(prev?.ae1_rhr)||parseFloat(r.ae1_rhr)||0)),
      ae2d: Math.max(0,(parseFloat(r.ae2_rhr)||0)-(parseFloat(prev?.ae2_rhr)||parseFloat(r.ae2_rhr)||0)),
      ae3d: Math.max(0,(parseFloat(r.ae3_rhr)||0)-(parseFloat(prev?.ae3_rhr)||parseFloat(r.ae3_rhr)||0)),
      ...
      <div className={CARD}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={wi} margin={{top:0,right:10,left:-10,bottom:0}}>
            ...
            <XAxis dataKey="label" tick={AX} interval="preserveStartEnd"/>
            <YAxis tick={AX}/><Tooltip contentStyle={TT}/><Legend wrapperStyle={{fontSize:11,color:'#94A3B8'}}/>
            <Bar dataKey="ae1d" name="AE1" stackId="a" fill="#6EE7B7"/>
            <Bar dataKey="ae2d" name="AE2" stackId="a" fill="#93C5FD"/>
            <Bar dataKey="ae3d" name="AE3" stackId="a" fill="#DDD6FE" radius={[3,3,0,0]}/>
          </BarChart>
        </ResponsiveContainer>
      </div>
      <div className="grid grid-cols-2 gap-6">
        <div className={CARD}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={wi} margin={{top:0,right:10,left:-10,bottom:0}}>
              ...
              <XAxis dataKey="label" tick={AX} interval="preserveStartEnd"/>
              <YAxis tick={AX}/><Tooltip contentStyle={TT}/>
              <Bar dataKey="cargo" name="Cargo Plant" fill="#34D399" radius={[3,3,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className={CARD}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={wi} margin={{top:0,right:10,left:-10,bottom:0}}>
              ...
              <XAxis dataKey="label" tick={AX} interval="preserveStartEnd"/>
              <YAxis tick={AX}/><Tooltip contentStyle={TT}/><Legend wrapperStyle={{fontSize:11,color:'#94A3B8'}}/>
              <Line type="monotone" dataKey="fw_rob"  name="FW ROB"      stroke="#34D399" strokeWidth={2} dot={false}/>
              <Line type="monotone" dataKey="fw_prod" name="FW Produced" stroke="#6EE7B7" strokeDasharray="4 2" strokeWidth={1} dot={false}/>
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div className={CARD}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={wi} margin={{top:0,right:10,left:-10,bottom:0}}>
              ...
              <XAxis dataKey="label" tick={AX} interval="preserveStartEnd"/>
              <YAxis tick={AX}/><Tooltip contentStyle={TT}/><Legend wrapperStyle={{fontSize:11,color:'#94A3B8'}}/>
              <Line type="monotone" dataKey="cyl_rob"  name="Cyl ROB"  stroke="#F59E0B" strokeWidth={2} dot={false}/>
              <Line type="monotone" dataKey="cyl_cons" name="Cyl Cons" stroke="#FDE68A" strokeDasharray="4 2" strokeWidth={1} dot={false}/>
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div className={CARD}>
    ...
    <div className="space-y-6">
      ...
      <div className={CARD}>
        ...
        <div className="grid grid-cols-5 gap-1 text-center text-[10px] mb-4">
          {[['A',`≤${f2(cii.bounds.A)}`,'#059669'],['B',`≤${f2(cii.bounds.B)}`,'#0891B2'],['C',`≤${f2(cii.bounds.C)}`,'#D97706'],['D',`≤${f2(cii.bounds.D)}`,'#EA580C'],['E',`>${f2(cii.bounds.D)}`,'#DC2626']].map(([l,v,c],i)=>(
...
// ── INLINE EDIT ROW ───────────────────────────────────────────────────────────
function LPGEditRow({record,onSave,onCancel,saving}) {
  ...
  return (
    <tr className="border-b border-amber-800/20 bg-amber-900/5">
      ...
      <td className="px-1 py-1"><input value={r.time_utc||''} onChange={e=>s('time_utc',e.target.value)} className="px-1 py-0.5 rounded bg-slate-700 border border-white/10 text-slate-200 text-xs w-12 focus:outline-none"/></td>
...
export function LPGImport() {
  ...
  const preview = async () => {
    ...
    try {
      ...
      setParsed(data); setSelected(data.voyages.map((_,i)=>i)); setStep('preview');
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setResults(data.results); setStep('done');
      ...
      {step==='upload' && (
        <div className={CARD}>
          <div className="border-2 border-dashed border-white/10 rounded-xl p-10 text-center cursor-pointer hover:border-amber-700/50 transition-colors"
            ...
            <div className="text-xs text-slate-500 mt-2">Alfred Temile daily noon counter format · all entries · grouped by month</div>
            ...
            {parsed.voyages.map((v,i)=>(
              <div key={i} className={`${CARD} ${selected.includes(i)?'border-amber-800/40':'opacity-50'} flex items-center gap-3`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-bold text-slate-100">{v.month_label||fML(v.voyage_number)}</div>
      ...
      <div className={CARD}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={trend} margin={{top:0,right:10,left:-10,bottom:0}}>
            <defs>
              <linearGradient id="gU" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#F59E0B" stopOpacity={0.4}/><stop offset="95%" stopColor="#F59E0B" stopOpacity={0}/></linearGradient>
              <linearGradient id="gL" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#67E8F9" stopOpacity={0.4}/><stop offset="95%" stopColor="#67E8F9" stopOpacity={0}/></linearGradient>
            </defs>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)"/>
            <XAxis dataKey="date" tick={AX} interval={intv} tickFormatter={v=>v?.slice(5)||''}/>
            <YAxis tick={AX}/><Tooltip contentStyle={TT}/><Legend wrapperStyle={{fontSize:11,color:'#94A3B8'}}/>
            <Area type="monotone" dataKey="ulsfo_cons" name="ULSFO" stroke="#F59E0B" fill="url(#gU)" strokeWidth={2} dot={false}/>
            <Area type="monotone" dataKey="lsmgo_cons" name="LSMGO" stroke="#67E8F9" fill="url(#gL)" strokeWidth={2} dot={false}/>
          </AreaChart>
        </ResponsiveContainer>
      </div>
