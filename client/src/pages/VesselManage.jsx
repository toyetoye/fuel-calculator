...
const defaults = { name: '', capacity_m3: '', dwt: '', foe_factor: '0.484', pitch: '', vessel_class: 'Rivers', laden_boiloff_pct: '0.15', ballast_boiloff_pct: '0.10', cf_hfo: '3.114', cf_foe: '2.750' };
...
export default function VesselManage() {
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        cf_hfo: parseFloat(form.cf_hfo) || 3.114,
        cf_foe: parseFloat(form.cf_foe) || 2.750,
      };
      if (editing) await api.updateLngVessel(editing.id, payload);
      else await api.createLngVessel(payload);
      setShowForm(false); setEditing(null); setForm({ ...defaults });
      loadVessels();
  ...
  const startEdit = (v) => {
    setEditing(v);
    setForm({
      name: v.name, capacity_m3: v.capacity_m3, dwt: v.dwt || '', foe_factor: v.foe_factor, pitch: v.pitch || '',
      vessel_class: v.vessel_class || 'Rivers', laden_boiloff_pct: v.laden_boiloff_pct, ballast_boiloff_pct: v.ballast_boiloff_pct,
      cf_hfo: v.cf_hfo || '3.114', cf_foe: v.cf_foe || '2.750'
    });
    setShowForm(true);
  };
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage vessel specifications for fuel and CII calculations</p>
        </div>
        {canEdit && <button onClick={() => { setEditing(null); setForm({ ...defaults }); setShowForm(true); }}
      ...
      {showForm && canEdit && (
        <div className="rounded-xl p-5 border border-amber-800/30 space-y-3" style={{ background: 'rgba(180,83,9,0.05)' }}>
          <h3 className="text-sm font-semibold text-amber-300">{editing ? 'Edit Vessel' : 'New Vessel'}</h3>
          <div className="grid grid-cols-4 gap-4">
            ...
            <div><label className={label}>DWT (MT) *</label><input type="number" value={form.dwt} onChange={e => setForm({ ...form, dwt: e.target.value })} className={inp} placeholder="e.g. 87362" />
              <span className="text-[10px] text-slate-600">Required for CII calculation</span></div>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div><label className={label}>CF HFO (t-CO₂/t)</label><input type="number" step="0.001" value={form.cf_hfo} onChange={e => setForm({ ...form, cf_hfo: e.target.value })} className={inp} />
              <span className="text-[10px] text-slate-600">IMO default: 3.114</span></div>
            <div><label className={label}>CF LNG/FOE (t-CO₂/t)</label><input type="number" step="0.001" value={form.cf_foe} onChange={e => setForm({ ...form, cf_foe: e.target.value })} className={inp} />
              <span className="text-[10px] text-slate-600">IMO default: 2.750</span></div>
          </div>
      ...
      <div className="rounded-xl border border-white/5 overflow-x-auto" style={{ background: 'rgba(15,23,42,0.6)' }}>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-white/5">
            {['Vessel', 'Class', 'Capacity (M³)', 'DWT', 'FOE', 'Pitch', 'Laden B/O', 'Ballast B/O', 'CF HFO', 'CF LNG', ''].map(h => (
              <th key={h} className="px-3 py-3 text-left text-xs text-slate-500 uppercase whitespace-nowrap">{h}</th>
            ))}
          </tr></thead>
          <tbody>{vessels.map(v => (
            <tr key={v.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
              <td className="px-3 py-3 text-sm font-semibold text-slate-200">{v.name}</td>
              <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs font-medium ${v.vessel_class === 'Rivers' ? 'bg-blue-900/30 text-blue-300' : 'bg-purple-900/30 text-purple-300'}`}>{v.vessel_class}</span></td>
              <td className="px-3 py-3 font-mono text-xs text-slate-300">{Number(v.capacity_m3).toLocaleString()}</td>
              <td className="px-3 py-3 font-mono text-xs text-slate-300">{v.dwt && Number(v.dwt) > 0 ? Number(v.dwt).toLocaleString() : <span className="text-red-400">Not set</span>}</td>
              <td className="px-3 py-3 font-mono text-xs text-slate-400">{v.foe_factor}</td>
              <td className="px-3 py-3 font-mono text-xs text-slate-400">{v.pitch || '—'}</td>
              <td className="px-3 py-3 font-mono text-xs text-slate-400">{v.laden_boiloff_pct}%</td>
              <td className="px-3 py-3 font-mono text-xs text-slate-400">{v.ballast_boiloff_pct}%</td>
              <td className="px-3 py-3 font-mono text-xs text-slate-400">{v.cf_hfo || '3.114'}</td>
              <td className="px-3 py-3 font-mono text-xs text-slate-400">{v.cf_foe || '2.750'}</td>
