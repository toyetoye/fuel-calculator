    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {canReview && !isFinalised && <button onClick={async () => { if (confirm('Finalise this voyage? This locks the calculation.')) { await api.finaliseVoyage(id); loadAll(); } }} className="px-3 py-1.5 rounded-lg text-xs text-emerald-300 bg-emerald-900/20 border border-emerald-800/30">Finalise</button>}
      ...
      <div className="flex gap-1 rounded-lg p-1 mb-6" style={{ background: 'rgba(255,255,255,0.04)' }}>
        {['reports', 'summary', 'foe'].map(t => (
          <button key={t} onClick={() => setTab(t)} className="px-4 py-1.5 rounded-md text-xs font-semibold uppercase tracking-wider"
            ...
            {t === 'reports' ? 'Noon Reports' : t === 'summary' ? 'Excess Fuel Summary' : 'FOE / Boil-Off'}
          </button>
        ))}
      </div>
