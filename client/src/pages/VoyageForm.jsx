  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="rounded-xl p-5 border border-white/5 space-y-5" style={{ background: 'rgba(15,23,42,0.6)' }}>
        ...
        <div className="grid grid-cols-2 gap-4">
          {F('HFO Price ($/MT)', <div>
            <input type="number" value={form.hfo_price} onChange={e => { if (['admin','superintendent'].includes(user?.role)) setForm({ ...form, hfo_price: e.target.value }); }} className={inp} readOnly={!['admin','superintendent'].includes(user?.role)} style={!['admin','superintendent'].includes(user?.role) ? { opacity: 0.6 } : {}} />
            ...
            {!['admin','superintendent'].includes(user?.role) && <div className="text-[10px] text-slate-600 mt-1">Only admin/superintendent can adjust the fuel price</div>}
          </div>)}
